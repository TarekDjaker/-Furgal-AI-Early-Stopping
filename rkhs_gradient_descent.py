...
def example_usage():
    ...
    ax.axvline(x=result_dp['tau'], color='b', linestyle='--', alpha=0.5, label=f"DP tau = {result_dp['tau']}")
    ax.axvline(x=result_sdp['tau'], color='r', linestyle='--', alpha=0.5, label=f"SDP tau = {result_sdp['tau']}")
    ...
    ax.scatter(X_train, Y_train, alpha=0.5, s=20, label='Training Data')
    ax.plot(X_test, Y_test_true, 'k-', linewidth=2, label='True Function')
    ax.plot(X_test, Y_pred_sdp, 'r--', linewidth=2, label=f'SDP Prediction (tau={result_sdp["tau"]})')
    ax.set_xlabel('X')
    ax.set_ylabel('Y')
